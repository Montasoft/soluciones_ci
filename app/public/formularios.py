...
from wtforms import Form, IntegerField, SelectField, SubmitField, StringField, PasswordField, BooleanField
...
#    correo = html5.EmailField("email", [validators.Required(message= 'El correo electronico es requerido'), validators.Email(message='Ingrese un email valido')])
    teleono = IntegerField("Nombre", [validators.InputRequired(message= 'El número de teléfono es requerido'),validators.length(min=7,max=10, message='Ingrese su número de teléfono')])
    ...
    mensaje = StringField("Mensaje", [validators.InputRequired(message= 'El mensaje es requerido'),validators.length(min=8,max=250, message='Ingrese el contenido de su mensaje')])
